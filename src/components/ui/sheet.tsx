...
const sheetVariants = cva(
	"fixed z-50 gap-4 bg-white text-sea-ink p-6 shadow-2xl transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-400 data-[state=open]:animate-in data-[state=closed]:animate-out",
	{
		variants: {
			side: {
...
function SheetContent({
	...
	return (
		<SheetPortal>
			...
			>
				{children}
				<SheetPrimitive.Close className="absolute top-4 right-4 rounded-full p-2 opacity-70 transition-opacity hover:opacity-100 focus:outline-hidden focus:ring-2 focus:ring-ring disabled:pointer-events-none bg-foam border border-line text-sea-ink hover:bg-sand">
					<XIcon className="size-4 text-sea-ink" />
...
function SheetTitle({
	...
	return (
		<SheetPrimitive.Title
			...
			className={cn("text-lg font-bold font-serif text-sea-ink", className)}
			{...props}
		/>
	);
}
